...
from robot_sim.sensor_utils import ImageUtils, SensorClient
...
class CameraViewer:
    ...
    def start_matplotlib(self, interval=33):
    ...
    def start(self, interval=33):
        """Start live viewer using OpenCV (no matplotlib)"""
        print("Waiting for first frame to detect cameras...")
        data = self.client.receive_message()

        # Detect camera names (same logic as before)
        camera_names = []
        if "images" in data and isinstance(data["images"], dict):
            camera_names = list(data["images"].keys())
        else:
        ...
        print(f"\n{'='*60}")
        print("📹 OpenCV Live Camera Viewer Started!")
        print("Press ESC in any camera window to exit")
        print(f"{'='*60}\n")
        ...
        while True:
            try:
                ...
                for cam_name in camera_names:
                    ...
                    # Optional: add FPS text on image
                ...
                # Check for ESC key (must call waitKey after imshow)
